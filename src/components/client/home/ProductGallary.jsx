...
function ProductGallary() {
    const { products, productCategorys } = useContext(ProductContext);
    const [categorizedProducts, setCategorizedProducts] = useState([]);
    const [categorie, setCategorie] = useState("beauty");
    ...
    useEffect(() => {
        ...
        setCategorizedProducts(updatedArray);
    }, [categorie]);

    return (
        <div>
            <div className="flex items-center justify-center py-4 md:py-8 flex-wrap">
                {productCategorys.map((categories, i) => (
                    <button
                        key={i}
                        onClick={() => setCategorie(categories.slug)}
                        type="button"
                        className={`${
                            categories.slug === categorie
                                ? "text-blue-700 border-blue-600 hover:text-white border  bg-white hover:bg-blue-700 focus:ring-4 focus:outline-none focus:ring-blue-300"
                        ...
                        }  rounded-full text-base font-medium px-5 py-2.5 text-center me-3 mb-3 dark:text-white dark:focus:ring-gray-800`}
                    >
                        {categories.name}
                    </button>
                ))}
                              ...
                              <div className="pt-6">
                                  <div className="mb-4 flex items-center justify-between gap-4">
                                      <span className="me-2 rounded bg-primary-100 px-2.5 py-0.5 text-xs font-medium text-primary-800 dark:bg-primary-900 dark:text-primary-300">
                                          Up to {product.discountPercentage}%
                                          off
                                      </span>
